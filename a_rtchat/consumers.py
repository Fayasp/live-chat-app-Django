from channels.generic.websocket import WebsocketConsumer  #handling the websocket connection
...
class ChatroomConsumer(WebsocketConsumer):
    ...
    def online_count_handler(self,event):
        ...
        auther_ids     = set ([message.author.id for  message  in chat_messages])
        users          = User.objects.filter(id__in = auther_ids)
        context = {
            "online_count" : online_count,
            "chat_group"   : self.chatroom,
            'users'        : users
...
class OnlineStatusConsumer(WebsocketConsumer):
    ...
    def online_status(self):
        event = {
            'type' : 'online_status_handler'   #online_status_handler will render the htmx partials for each users
        } 
    ...
    def online_status_handler(self,event):
        ...
        group_chat_users         = [chat for chat in my_chats.filter(groupchat_name__isnull = False) if chat.users_online.exclude(id = self.user.id)]

        if public_chat_users or private_chat_with_users or group_chat_users:
